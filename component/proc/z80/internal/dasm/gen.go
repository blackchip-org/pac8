...
`)
	...
	// unprefixed
	out.WriteString("var dasmTable = map[uint8]func(proc.Eval){\n")
	...
	// dd prefix
	out.WriteString("var dasmTableDD = map[uint8]func(proc.Eval){\n")
	for i := lineStart; i <= lineEnd; i++ {
		line := lines[i]
	...
	// fd prefix
	out.WriteString("var dasmTableFD = map[uint8]func(proc.Eval){\n")
	for i := lineStart; i <= lineEnd; i++ {
		line := lines[i]
	...
	// cb prefix
	out.WriteString("var dasmTableCB = map[uint8]func(proc.Eval){\n")
	for i := lineStart; i <= lineEnd; i++ {
		line := lines[i]
	...
	// fd cb prefix
	out.WriteString("var dasmTableFDCB = map[uint8]func(proc.Eval){\n")
	for i := lineStart; i <= lineEnd; i++ {
		line := lines[i]
	...
	// dd cb prefix
	out.WriteString("var dasmTableDDCB = map[uint8]func(proc.Eval){\n")
	for i := lineStart; i <= lineEnd; i++ {
		line := lines[i]
	...
	// ed prefix
	out.WriteString("var dasmTableED = map[uint8]func(proc.Eval){\n")
	for i := lineStart; i <= lineEnd; i++ {
		line := lines[i]
...
func parseTable(out *bytes.Buffer, line string, firstBreak int, prefix string) {
	...
	case prefix == "" && opcode == 0xcb:
		out.WriteString("0xcb: func(e proc.Eval) { opCB(e) },\n")
		return
	case prefix == "" && opcode == 0xdd:
		out.WriteString("0xdd: func(e proc.Eval) { opDD(e) },\n")
		return
	case prefix == "" && opcode == 0xed:
		out.WriteString("0xed: func(e proc.Eval) { opED(e) },\n")
		return
	case prefix == "" && opcode == 0xfd:
		out.WriteString("0xfd: func(e proc.Eval) { opFD(e) },\n")
		return
	case
	...
	if prefix == "ddcb" || prefix == "fdcb" {
		out.WriteString(": func(e proc.Eval) { op2(e, ")
	} else {
		out.WriteString(": func(e proc.Eval) { op1(e, ")
	}
...
`)
	...
	for i := 0; i < len(lines); i++ {
		...
		i++
		op := lines[i]
		out.WriteString(fmt.Sprintf(`Test{"%v", "%v", []uint8{%v}},`, strdata, op, hexdata))
		out.WriteString("\n")
	}
	out.WriteString("}\n")

	err = ioutil.WriteFile("harston.go", out.Bytes(), 0644)
	if err != nil {
		fmt.Printf("unable to write file: %v", err)
		os.Exit(1)
	}
